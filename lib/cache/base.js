...
function Cache(impromptu, name, options) {
  ...
  this.impromptu.config.on('refreshCache', function (needsRefresh) {
 ...
 */
Cache.Error = function (message) {
  BaseError.apply(this, arguments)
}
util.inherits(Cache.Error, BaseError)
...
Cache.prototype._setThenGet = function(done) {
  this.set(function(err, results) {
    if (err && !(err instanceof Cache.Error)) {
      if (done) {
        done(err)
      }
    } else {
 ...
 */
Cache.prototype._claimSetLock = function(callback) {
 ...
 */
Cache.prototype._releaseSetLock = function(err, success) {
