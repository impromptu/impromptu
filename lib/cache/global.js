 ...
 */
function Global(impromptu, name, options) {
  Cache.call(this, impromptu, name, options)
}
...
Global.prototype.set = function(fn) {
  ...
  async.waterfall([
    ...
    function(exists, done) {
      if (exists) {
        done(new Cache.Error('The cache is currently locked.'))
      } else {
    ...
    function(pid, done) {
      if (pid && processIsRunning(pid)) {
        ...
        done(new Cache.Error('A process is currently updating the cache.'))
